...
CV = StratifiedKFold(n_splits=5, shuffle=True, random_state=11)
svm = SVC(gamma='scale', kernel='linear', random_state=11)
#et = ExtraTreesClassifier(n_estimators=200, criterion='gini', max_depth=32, random_state=11, n_jobs=-1)
...
FILE_NAME = 'top_{0}_to_{1}_step_{2}_spssm_cpssm_spd3_qso_ngram_pngram_smote_test_performances_svm_final6.xlsx'.format(start,stop,step)
TopFeaturesPerformance = os.path.join(PSSM_TRAIN_PATH, FILE_NAME)

for lag in Franges:
    DataSet = []
    COLUMNs = None
    #cls = ExtraTreesClassifier(n_estimators=200, criterion='gini', max_depth=32, random_state=11, n_jobs=-1)
