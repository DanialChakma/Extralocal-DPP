...
BestFeaturePath = os.path.join(PSSM_TRAIN_PATH, 'spssm_cpssm_spd3_qso_ngram_pngram_features1.csv')
...
if not os.path.exists(BestFeaturePath):
    ...
    for step in GRAINS:
        ...
        C_X = np.asarray(TestDF[selected_features].values, dtype=np.float64)
        rfs.fit(X=C_X,y=TestY)

        if np.sum(rfs.support_) < 1024:
...
rfs = RFECV_CBR(estimator=et, step=16, cv=CV, CBR=False, Tg=2, Tc=0.90, scoring='balanced_accuracy', n_jobs=-1, verbose=1)

print('Feature Selection Started....')
_, FC = X.shape
rfs.fit(X=X_TEST, y=Y_TEST)
...
FILE_NAME = 'top_{0}_to_{1}_step_{2}_spssm_cpssm_spd3_qso_ngram_pngram_smote_test_performances_ET_final.xlsx'.format(start,stop,step)
TopFeaturesPerformance = os.path.join(PSSM_TRAIN_PATH, FILE_NAME)

for lag in Franges:
    DataSet = []
    COLUMNs = None
    cls = ExtraTreesClassifier(n_estimators=100, criterion='gini', max_depth=32, random_state=11, n_jobs=-1)
